...
const CreatorDisplay: React.FC<CreatorDisplayProps> = ({ creator, displayTime, timeTooltip, onGotoDetail, variant = "x" }) => {
  if (variant === "x") {
    return (
      <div className="flex min-w-0 flex-wrap items-center gap-1 text-[15px] leading-5">
        <Link className="truncate font-bold text-foreground hover:underline" to={`/u/${encodeURIComponent(creator.username)}`} viewTransition>
          {creator.displayName || creator.username}
        </Link>
        <Link className="truncate text-muted-foreground hover:underline" to={`/u/${encodeURIComponent(creator.username)}`} viewTransition>
          @{creator.username}
        </Link>
        <span className="text-muted-foreground">·</span>
        <TimeTooltip content={timeTooltip}>
          <span className="cursor-pointer truncate text-muted-foreground hover:underline" onClick={onGotoDetail}>
            {displayTime}
          </span>
        </TimeTooltip>
      </div>
...
const TimeDisplay: React.FC<TimeDisplayProps> = ({ displayTime, timeTooltip, onGotoDetail, variant = "x" }) => {
  if (variant === "x") {
    return (
      <TimeTooltip content={timeTooltip}>
        <span className="cursor-pointer text-[15px] font-bold text-foreground hover:underline" onClick={onGotoDetail}>
          {displayTime}
        </span>
      </TimeTooltip>
    );
